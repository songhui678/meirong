...
// Avoid potential rounding error
	...
	function SetLineStyle($s) {
		if (is_numeric($s)) {
			if ($s < 1 || $s > 4) {
				JpGraphError::RaiseL(25101, $s);
			}
//(" Illegal numeric argument to SetLineStyle(): ($s)");
		} elseif (is_string($s)) {
			if ($s == "solid") {
				$s = 1;
			} elseif ($s == "dotted") {
				...
				JpGraphError::RaiseL(25102, $s);
			}
//(" Illegal string argument to SetLineStyle(): $s");
		} else {
			JpGraphError::RaiseL(25103, $s);
		}
	...
	function Stream($aFile = "") {
		...
		if ($this->img_format == "jpeg" && $this->quality != null) {
			...
			if ($aFile != "") {
				...
				if (!$res) {
					JpGraphError::RaiseL(25107, $aFile);
				}
//("Can't write to file '$aFile'. Check that the process running PHP has enough permission.");
			} else {
				$res = @$func($this->img);
				if (!$res) {
					JpGraphError::RaiseL(25108);
				}
		...
		if (USE_CACHE && file_exists($aCacheFileName) && $this->timeout >= 0) {
			...
			if ($this->timeout > 0 && ($diff > $this->timeout * 60)) {
				...
				if ($fh = @fopen($aCacheFileName, "rb")) {
					...
					JpGraphError::RaiseL(25117, $aCacheFileName);
				}
//(" Can't open cached image \"$aCacheFileName\" for reading.");
			}
		}
		return false;
	}
...
//(" Can't create directory $aFile. Make sure PHP has write permission to this directory.");
			...
			if (CACHE_FILE_GROUP != "") {
				...
				if (!$res) {
					JpGraphError::RaiseL(25119, $aFile);
				}
